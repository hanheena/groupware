...
var calendar = $('#calendar').fullCalendar({
	...
	eventRender: function(event, element, view) {
		...
		element.popover({
			...
			}).append('<p><strong>등록자:</strong> ' + event.write_user + '</p>')
...
/*function filtering(event) {
  ...
  show_username = username.indexOf(event.write_user) >= 0;

  if (types && types.length > 0) {
	if (types[0] == "all") {
	  show_type = true;
	} else {
	  show_type = types.indexOf(event.event_type) >= 0;
	}
  }
  
  console.log("show_username 확인 : " + show_username);
  console.log("show_type 확인 : " + show_type);
...
function filtering(event) {
	...
	/*console.log("show_type 확인 : " + show_type);
	console.log("type.indexOf(event.event_type) 확인 : " + type.indexOf(event.event_type));*/
...
function calDateWhenDragnDrop(event) {
	...
	//2일이상 all day
	else if (event.all_day && event.end !== null) {
		newDates.startDate = moment(event.start._d).format('YYYY-MM-DD');
		newDates.endDate = moment(event.end._d).subtract(1, 'days').format('YYYY-MM-DD');
	}
